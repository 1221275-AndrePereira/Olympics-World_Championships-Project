...
public interface IClassificationRepository
{
    ...
    Task<List<ClassificationEntry>> GetMedalEntriesAsync(string season, int? year = null);

    /// All medal-position entries for a season, for one specific country.
    /// If `year` is provided, the results are limited to that Games year.
    Task<List<ClassificationEntry>> GetMedalEntriesForCountryAsync(string season, string country, int? year = null);
