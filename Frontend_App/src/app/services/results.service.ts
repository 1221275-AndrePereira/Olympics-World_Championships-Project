...
export class ResultsService {
  ...
  getSports(season: string): Observable<string[]> {
    return this.http.get<string[]>(`${API_BASE}/browse/sports`, { params: { season } });
  }
 
  getEvents(season: string, sport: string): Observable<EventOption[]> {
    return this.http.get<EventOption[]>(`${API_BASE}/browse/events`, { params: { season, sport } });
  }
 
  getResults(
    season: string, sport: string, eventKey: string,
    athlete: string, country: string, sortBy: SortBy
  ): Observable<ResultRow[]> {
    let params = new HttpParams().set('season', season).set('sport', sport).set('eventKey', eventKey).set('sortBy', sortBy);
  ...
  getCountryMedalists(
    season: string, year: number, country: string, athlete: string, sortBy: SortBy
  ...
  getAllTimeCountryMedalists(
    season: string, country: string, athlete: string, sortBy: SortBy
  ): Observable<CountryMedal[]> {
